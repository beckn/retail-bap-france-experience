...
const state = reactive({
  ...
  openmodal: true,
  openTrekModal: false,
  pastedOrderObject: null
});

const useUiState = () => {
  const openmodal = computed(() => state.openmodal);
  const pastedOrderObject = computed(() => state.pastedOrderObject);
  const openTrekModal = computed(() => state.openTrekModal);

  const isopenmodal = () => {
    state.openmodal = !state.openmodal;
  };

  const isOpenTrekModal = async () => {
    try {
      // const pastedText = await navigator.clipboard.readText();
      // console.log('parsedTest', pastedText)
      // localStorage.setItem('copiedOrderObject', JSON.stringify(pastedText));
      // state.pastedOrderObject = JSON.parse(pastedText);

      state.openTrekModal = !state.openTrekModal;
    } catch (error) {
      alert('Please import valid order object');
    }
  };
