<template>
  <div class="sf-accordion-item no-border">
    ...
    > -->
    <SfButton :aria-pressed="isOpen.toString()" :aria-expanded="isOpen.toString()" :class="{ 'is-open': isOpen }"
      class="sf-button--pure sf-accordion-item__header no-border padding-head" @click="accordionClick">
      {{ header }}
      <!-- @slot here you can add additional information about this item -->
      <!-- <slot name="additional-info" /> -->
      <SfChevron tabindex="0" class="sf-accordion-item__chevron" :class="{ 'sf-chevron--right': !isOpen }" />
    </SfButton>
    <!-- </slot> -->
